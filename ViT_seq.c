...
void Conv2d(float* input, float* output, Network weight, Network bias) {
    ...
    int total = embed_dim * output_size * output_size;
    int idx;

    /*#pragma omp parallel for*/
    for (idx = 0; idx < total; ++idx) {
        int oc = idx / (output_size * output_size);
        int rem = idx % (output_size * output_size);
        int oh = rem / output_size;
        int ow = rem % output_size;
...
void multihead_attn(float* input, float* output,
    Network in_weight, Network in_bias, Network out_weight, Network out_bias, AttnWorkspace* ws) {
    ...
    for (t = 0; t < tokens; t++) {
        for (i = 0; i < embed_dim; i++) {
            ...
            for (int j = 0; j < embed_dim; j++) {
                ...
                sum_q += x * in_weight.data[(Q_dim + i) * embed_dim + j];
                sum_k += x * in_weight.data[(K_dim + i) * embed_dim + j];
                sum_v += x * in_weight.data[(V_dim + i) * embed_dim + j];
            }
    ...
    /*Attn 결과를 저장할 버퍼*/
    float* attn_output = ws->attn_output;
    ...
    for (int h = 0; h < num_heads; h++) {
        ...
        // attn_score 저장 공간
        float* scores = ws->scores + h * tokens * tokens;
        float* head_out = ws->head_out + h * tokens * head_dim;
        ...
        for (i = 0; i < tokens; i++) {
            for (j = 0; j < tokens; j++) {
                ...
                scores[i * tokens + j] = score * inv_sqrt_head_dim;
            }
        }
        ...
        for (i = 0; i < tokens; i++) {
            ...
            for (int j = 1; j < tokens; j++) {
                float v = scores[i * tokens + j];
                if (v > max_val) max_val = v;
            }
            float sum_exp = 0.0f;
            for (int j = 0; j < tokens; j++) {
                float e = expf(scores[i * tokens + j] - max_val);
                scores[i * tokens + j] = e;
                sum_exp += e;
            }
            float inv_sum = 1.0f / sum_exp;
            for (int j = 0; j < tokens; j++) {
                scores[i * tokens + j] *= inv_sum;
            }
        }
...
void linear_layer(float* input, float* output, int tokens, int in_features, int out_features, Network weight, Network bias) {
    ...
    for (t = 0; t < tokens; t++) {
        for (o = 0; o < out_features; o++) {
            ...
            float* w = &weight.data[o * in_features];
            float* x = &input[t * in_features];
...
void Encoder(float* input, float* output,
    ...
    Network ln2_w, Network ln2_b, Network mlp1_w, Network mlp1_b, Network mlp2_w, Network mlp2_b, AttnWorkspace* ws) {
    int tokens = ((img_size / patch_size) * (img_size / patch_size)) + 1;
    ...
    /*Attn*/
    multihead_attn(ln1_out, attn_out, attn_w, attn_b, attn_out_w, attn_out_b, ws);
...
void ViT_seq(ImageData* image, Network* networks, float** probabilities) {
    ...
    ws.Q = (float*)malloc(sizeof(float) * tokens * embed_dim);
    ws.K = (float*)malloc(sizeof(float) * tokens * embed_dim);
    ws.V = (float*)malloc(sizeof(float) * tokens * embed_dim);
    ws.scores = malloc(sizeof(float) * num_heads * tokens * tokens);
    ws.head_out = malloc(sizeof(float) * num_heads * tokens * head_dim);
    ws.attn_output = (float*)malloc(sizeof(float) * tokens * embed_dim);
    ...
    for (int i = 0; i < image->n; i++) {
        ...
        Encoder(layer[3], enc_layer[0],
            ...
            networks[12], networks[13], networks[14], networks[15], &ws);
        ...
        Encoder(enc_layer[0], enc_layer[1],
            ...
            networks[24], networks[25], networks[26], networks[27], &ws);
        ...
        Encoder(enc_layer[1], enc_layer[2],
            ...
            networks[36], networks[37], networks[38], networks[39], &ws);
        ...
        Encoder(enc_layer[2], enc_layer[3],
            ...
            networks[48], networks[49], networks[50], networks[51], &ws);
        ...
        Encoder(enc_layer[3], enc_layer[4],
            ...
            networks[60], networks[61], networks[62], networks[63], &ws);
        ...
        Encoder(enc_layer[4], enc_layer[5],
            ...
            networks[72], networks[73], networks[74], networks[75], &ws);
        ...
        Encoder(enc_layer[5], enc_layer[6],
            ...
            networks[84], networks[85], networks[86], networks[87], &ws);
        ...
        Encoder(enc_layer[6], enc_layer[7],
            ...
            networks[96], networks[97], networks[98], networks[99], &ws);
        ...
        Encoder(enc_layer[7], enc_layer[8],
            ...
            networks[108], networks[109], networks[110], networks[111], &ws);
        ...
        Encoder(enc_layer[8], enc_layer[9],
            ...
            networks[120], networks[121], networks[122], networks[123], &ws);
        ...
        Encoder(enc_layer[9], enc_layer[10],
            ...
            networks[132], networks[133], networks[134], networks[135], &ws);
        ...
        Encoder(enc_layer[10], enc_layer[11],
            ...
            networks[144], networks[145], networks[146], networks[147], &ws);
