    ...
    cl_context context, cl_command_queue queue, cl_program program, cl_device_id device)
{
    ...
    for (int i = 0; i < image->n; i += BATCH_SIZE) {
        ...
        for (int l = 0; l < 12; l++) {
            ...
            size_t gws_score[3] = { (size_t)current_batch * NUM_HEADS, (size_t)tokens_per_img, (size_t)tokens_per_img };
            ...
            size_t gws_softmax[2] = { (size_t)current_batch * NUM_HEADS, (size_t)tokens_per_img };
            ...
            size_t gws_val[3] = { (size_t)current_batch * NUM_HEADS, (size_t)tokens_per_img, (size_t)HEAD_DIM / 4 };
            clEnqueueNDRangeKernel(queue, k_attn_val, 3, NULL, gws_val, NULL, 0, NULL, NULL);

            // MHA - Final Linear
            set_linear_args(k_linear, d_buf2, d_residual, d_networks[net_idx + 4], d_networks[net_idx + 5],
            ...
            clSetKernelArg(k_ln, 1, sizeof(cl_mem), &d_buf2);                 // 출력: ln2 결과
            ...
            // MLP FC1
            set_linear_args(k_linear, d_buf2, d_intermediate, d_networks[net_idx + 8], d_networks[net_idx + 9],
            ...
            // MLP FC2
            set_linear_args(k_linear, d_intermediate, d_residual, d_networks[net_idx + 10], d_networks[net_idx + 11],
        ...
        set_linear_args(k_linear, d_cls_gathered, d_cls_out, d_networks[150], d_networks[151],
